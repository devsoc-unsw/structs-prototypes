...
function generateRandomArrayData(minValue: number, maxValue: number, length: number): ArrayDataStructure {
  const getRandomValue = (min: number, max: number): number => Math.floor(Math.random() * (max - min + 1)) + min;
  const getAddress = (index: number): string => `0x${(1000000 + index * 4).toString(16)}`;

  const data: ArrayNode[] = [];

  for (let i = 0; i < length; i++) {
    data.push({
      addr: { value: `arrayElement${i}`, addr: getAddress(i) },
